...
export const getAllProducts = async (req, res) => {
    try {
        const productList = await Product.find({ isActive: true }).select('title image price')
...
export const createProduct = async (req, res) => {
    ...
    try {
        ...
        const uploadResult = await cloudinary.uploader.upload(req.file.path, { folder: 'lapStore' });
        ...
        const product = await Product.create({ title, description, image: uploadResult.url, category, price, countInStock });
...
export const deactivateProduct = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: 'Product deactivated successfully', data: product });
    } catch (error) {
        console.log(error);
        res.status(error.status || 500).json({ message: error.message || 'Internal server error' });
    }
